...
export default async function PropertyDetailPage({
  ...
  return (
    <section className="container grid gap-8 py-10 lg:grid-cols-[1fr_360px]">
      <article>
        <div className="aspect-[16/9] rounded-lg border border-black/10 bg-white p-8 shadow-soft">
        ...
        {property.images.length > 1 ? (
          <div className="mt-4 grid gap-3 md:grid-cols-4">
            {property.images.slice(1, 5).map((image) => (
              <div key={image.id} className="relative aspect-[4/3] overflow-hidden rounded-lg border border-black/10 bg-neutral-100">
        ...
        <div className="mt-7">
          <p className="text-sm font-bold uppercase text-brand-red">{property.operation === "sale" ? "Venta" : "Alquiler"}</p>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <a
              ...
              className="inline-flex h-11 items-center gap-2 rounded-md bg-green-600 px-4 font-bold text-white"
            >
              <MessageCircle size={18} /> WhatsApp
            </a>
            <button className="inline-flex h-11 items-center gap-2 rounded-md border border-black/15 px-4 font-bold" type="button">
