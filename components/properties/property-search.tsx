...
export function PropertySearch({ filters = {} }: { filters?: PropertyFilters }) {
  return (
    <form className="rounded-lg border border-black/10 bg-white p-4 shadow-soft">
      <div className="grid gap-3 md:grid-cols-[1.4fr_repeat(5,1fr)_auto]">
        <label className="grid gap-1 text-sm font-semibold text-neutral-700">
          Zona
          <span className="relative">
            <Search className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-neutral-400" size={16} />
            <input className="h-11 w-full rounded-md border border-black/10 pl-9 pr-3" name="zone" placeholder="Municipio o barrio" defaultValue={filters.zone ?? ""} />
          </span>
        </label>
        ...
        <button className="mt-auto inline-flex h-11 items-center justify-center gap-2 rounded-md bg-brand-red px-4 font-bold text-white" type="submit">
...
function Checkbox({ name, value, label, checked = false }: { name: string; value: string; label: string; checked?: boolean }) {
  return (
    <label className="flex min-h-10 items-center gap-2 rounded-md bg-neutral-100 px-3 text-sm font-semibold text-neutral-700">
      <input name={name} type="checkbox" value={value} defaultChecked={checked} />
      {label}
    </label>
  );
}
